...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if zaxisshift then
          slope := (Pplayer_t(source.player).lookupdown * (FRACUNIT div 16)) div 173
        else
          slope := 0;
      end;
    end;
  ...
  if zaxisshift then
    z := source.z + 4 * 8 * FRACUNIT + (Pplayer_t(source.player).lookupdown * (FRACUNIT div 16)) div 173
  else
...
begin
  ...
  case P_GetThingFloorType(thing) of
    FLOOR_WATER:
      begin
      // JVAL
      // If we record a demo or playing demo
